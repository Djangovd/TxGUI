...
#    def __init__(self, i_in, i_out, ver=00000001, locktime=0):
    ...
    def add_input(self, new_in_prev_out_hash, new_in_prev_out_idx, new_in_script_length, new_in_script_sig, new_in_sequence): # add another input block, new_in_script_length obsolete for now
        in_script_len=script_length(new_in_script_sig,True)
        self.inputcount += 1
    ...
    def add_signature(self, i_in, new_script_sig, PubKey, tmp): # add signature(s) to input with sequence id
        newPubKeyLen = ''
        if not tmp:
            newPubKeyLen = script_length(PubKey) if PubKey != '' else ''
            newScriptSig = new_script_sig+str(newPubKeyLen)+PubKey
            print"scrpt = " + str(newScriptSig)
            newSigLen = script_length(newScriptSig,True)
            print"scrpt = " + str(newScriptSig)
            newScriptSig = newSigLen+newScriptSig
            print"scrpt = " + str(newScriptSig)
            self.in_script_length[int(i_in)] = newSigLen
            print"Length = " + str(self.in_script_length[int(i_in)])
            self.in_script_sig[int(i_in)] = new_script_sig + str(newPubKeyLen) + PubKey
        else:
    ...
    def dump(self): # print tx 
        ...
        for i in range(int(self.inputcount)):
            ...
            if str(self.in_script_length[i]) != '00': in_script = str(self.in_script_sig[i])
            print "      | script signature : " + str(self.in_script_length[i]) + in_script
